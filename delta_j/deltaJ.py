...
def main():
    ...
    columns = ['Potential (V)', 'Dark Current (A)',
               'On Current (A)', 'Delta Current (A)']
    ...
    ax.set_title(
        'Select the potential range that contains the on/off current\nClose the graph when finished selecting points')
    rectprops = dict(facecolor='blue', alpha=0.4)
    span = mwidgets.SpanSelector(
        ax, onselect, 'horizontal', rectprops=rectprops)
    plt.show()

    # Double checking selected points with graph
    plt.plot(data['Potential/V'], data[' Current/A'], color='blue')
    plt.scatter(output_data['Potential (V)'], output_data['Dark Current (A)'],
                label='Off Current', marker='o', color='red')
    plt.scatter(output_data['Potential (V)'], output_data['On Current (A)'],
                label='On Current', marker='o', color='darkorange')
